...
static char board[BOARD_SIZE][BOARD_SIZE]; // int currentPermutation[MAX_PERMUTATION_SIZE+1]

// void initializeCurrentPath()
static void initializeBoard();

// void attempt(int n, int level, int possibility)
static void attempt(int column, int numQueens, int* numSolutions);

// void addToCurrentPath(int level, int possibility)
static void placeQueen(int row, int column);

// void removeFromCurrentPath(int level)
static void removeQueen(int row, int column);

// bool currentPathSuccess(int n, int level)
static bool allQueensPlaced(int column, int numQueens);

// bool currentPathStillViable(int level)
static bool queenIsSafe(int row, int column);
